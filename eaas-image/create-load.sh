...
do_get() {
    ...
    curl -4 --connect-timeout 10 --retry 6 --retry-connrefused -sSf -o /dev/null "$1"
}

# Does a POST request to the given URL, then sleeps 1 second and outputs the URL the server redirects to
do_post() {
    echo "POST $1: $2" >&2
    curl -4 --connect-timeout 10 --retry 6 --retry-connrefused -sSf -w '%{redirect_url}' -X POST -d "$2" "$1"
    sleep 1
}
...
# Keep creating some load indefinitely
